...
(function (global) {
   ...
   */
  // 加粗部分用 (?:[^*]|\*(?!\*))+ 而不是 [^*]+ ——
  // 后者不允许内容里出现 *，于是 **`p` 和 `*p` 的区别** 这种
  // 正文里带星号的（C++ 里到处是 *）就匹配不上，** 会原样显示出来
  var INLINE_RE = /(`[^`\n]+`|\*\*(?:[^*]|\*(?!\*))+\*\*)/g;
  ...
  TYPES.cards = function (s) {
    ...
    if (s.code) inner.appendChild(anim(codePanel(s.code), 'rise', (s.cards || []).length + 3));
    return inner;
  };
  ...
  TYPES.split = function (s) {
    ...
    if (s.code) main.appendChild(anim(codePanel(s.code), 'right', 1));
  ...
  TYPES.code = function (s) {
    ...
    if (s.code) inner.appendChild(anim(codePanel(s.code), 'rise', 2));
  ...
  /* ---- memory：内存与指针可视化 ---- */
  TYPES.memory = function (s) {
    var inner = el('div', 'slide-inner');
    inner.appendChild(anim(head(s), 'rise', 0));

    // 有代码时左右分栏（代码在左、内存图在右，便于对照）；
    // 没有代码时让内存图独占整行 —— 否则左栏空着，图被挤到右边很难看
    var hasSide = !!(s.points || s.code);

    if (!hasSide) {
      var hostFull = el('div');
      hostFull.setAttribute('data-memory', '');
      inner.appendChild(anim(hostFull, 'rise', 1));
      if (s.note) inner.appendChild(anim(noteBox(s.note), 'rise', 2));
      inner._memoryHost = hostFull;
      return inner;
    }
    ...
    if (s.code) side.appendChild(anim(codePanel(s.code), 'left', 4));
    wrap.appendChild(side);

    var host = el('div');
    host.setAttribute('data-memory', '');
    main.appendChild(anim(host, 'right', 1));
    if (s.note) main.appendChild(anim(noteBox(s.note), 'right', 2));
    wrap.appendChild(main);

    inner.appendChild(wrap);
    inner._memoryHost = host;
    return inner;
  };
   ...
   * 提示卡。kind 决定视觉语言：
   *   note（默认）— 一般补充说明，蓝色
   *   trap        — C++ 陷阱：野指针、越界、未定义行为，红色
   *   env         — 环境/运行说明，琥珀色
   * C++ 教学里陷阱极多，必须与普通说明在视觉上区分开。
   */
  function noteBox(n) {
    if (typeof n === 'string') n = { text: n };
    var kind = n.kind || 'note';
    var box = el('div', kind === 'trap' ? 'trap' : (kind === 'env' ? 'env-note' : 'note-card'));

    var ic = el('span');
    ic.innerHTML = ICON(n.icon || (kind === 'trap' ? 'alert' : (kind === 'env' ? 'terminal' : 'bulb')));
    box.appendChild(ic);

    var body = el('div');
    if (n.title) body.appendChild(el('strong', null, n.title));
    if (n.title && n.text) body.appendChild(document.createTextNode('　'));
    body.appendChild(richText(el('span'), n.text || ''));
   ...
   * 代码面板：文件名 + 代码 + 真实编译输出。
   *
   * 与 Python 版最重要的差别：这里**没有运行按钮**，输出是静态展示的。
   * 原因是浏览器里没有可直接使用的 C++ 编译器（Pyodide 没有对等物），
   * 与其做一个假的"运行"，不如如实展示真实编译结果，
   * 并提供「在 Compiler Explorer 打开」让想动手的人去真实环境验证。
   */
  function codePanel(c) {
    ...
    headEl.appendChild(el('span', 'code-file', c.file || 'main.cpp'));

    var actions = el('div', 'code-panel-actions');
    actions.setAttribute('data-code-actions', '');
    if (c.explorer !== false) {
      // 汇编那一页要链接的是「生成它的 C++」，不是汇编本身 ——
      // 读者点过去是想看编译器怎么把 C++ 变成这段汇编的
      actions.appendChild(mkExternalLink('btn-ce', 'compile',
        c.explorerTitle || '在 Compiler Explorer 打开', 'explorer'));
    }
    actions.appendChild(mkBtn('btn-copy', 'copy', '复制', 'copy'));
    actions.appendChild(mkBtn('btn-reveal', 'check', '显示全部', 'reveal'));
    headEl.appendChild(actions);
    panel.appendChild(headEl);

    var block = global.PYT.code.render(c.source, { byLine: c.byLine, lang: c.lang });
    panel.appendChild(block);
    panel._codeBlock = block;

    // 程序输出：默认就展示，不需要点任何按钮。
    // 用 role="log" 让屏幕阅读器按日志语义播报，而不是整块重读。
    if (c.expectedOutput != null || c.outputNote) {
      var term = el('div', 'term');
      term.setAttribute('data-term', '');
      var head = el('div', 'term-head');
      head.appendChild(el('span', null, '程序输出'));
      var badge = el('span', 'term-badge');
      badge.innerHTML = ICON('check');
      badge.appendChild(el('span', null, '真实编译输出'));
      badge.title = '由 clang++ 实际编译运行得到，不是手写的预期值';
      head.appendChild(badge);
      term.appendChild(head);

      var body = el('div', 'term-body');
      body.setAttribute('data-term-body', '');
      body.setAttribute('role', 'log');
      body.setAttribute('aria-label', '程序输出');
      if (c.expectedOutput) {
        var lines = String(c.expectedOutput).replace(/\n$/, '').split('\n');
        for (var i = 0; i < lines.length; i++) {
          var ln = el('span', 'term-line');
          ln.textContent = lines[i];
          body.appendChild(ln);
        }
      } else {
        body.appendChild(el('span', 'term-empty', c.outputNote || '（本程序没有输出）'));
      }
      term.appendChild(body);
      panel.appendChild(term);
    }

    if (c.note) panel.appendChild(noteBox(c.note));
  ...
  /** 外链按钮。注意不能叫 mkLink —— 知识地图里已有一个同名函数画 SVG 连线，
      函数提升会让后定义者覆盖前者，导致这里被当成设 d 属性而报错。 */
  function mkExternalLink(cls, iconName, label, action) {
    var a = el('a', 'btn ' + cls);
