...
(function (global) {
  ...
  function renderChapters() {
    ...
    PYT.manifest.forEach(function (ch, i) {
      ...
      top.appendChild(ic);
      var meta = el('div');
      meta.appendChild(el('div', 'ch-num', ch.label));
  ...
  function showFileWarning() {
    ...
    body.appendChild(el('strong', null, '提示：建议通过本地服务器打开'));
    body.appendChild(el('br'));
    body.appendChild(document.createTextNode(
      '你现在是直接打开文件（file://）。幻灯片、动画、内存演示都能正常看，'
    ));
    body.appendChild(el('br'));
    body.appendChild(document.createTextNode('但 Compiler Explorer 跳转等外链功能可能受限。在 web 目录下执行 '));
    var c = el('code', null, 'python3 serve.py');
    body.appendChild(c);
    body.appendChild(document.createTextNode(' 后访问 http://localhost:8000 体验更完整。'));
  ...
  function boot() {
    ...
    document.getElementById('brand-icon').innerHTML = PYT.icons.get('cpp');
    ...
    document.getElementById('hero-meta').textContent =
      document.getElementById('hero-meta').textContent ||
      ('共 ' + PYT.manifest.length + ' 章 · ' + total + ' 页 · 全部带讲师讲解与真实编译输出');
  }
