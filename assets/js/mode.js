...
(function (global) {
  ...
  var mode = {
    ...
    record: function (chapterNum, slideIndex, slideCount) {
      ...
      if (!prev || slideIndex > prev.slide) {
        prog[key] = { slide: slideIndex, total: slideCount, at: Date.now() };
    ...
    /** 取某章的续读位置，没有则返回 0 */
    resume: function (chapterNum, slideCount) {
      var p = this.all()['ch' + chapterNum];
      if (!p || typeof p.slide !== 'number') return 0;
      // 章节改过（页数对不上）就从头看：旧页码对应的已经不是同一页了 ——
      // 比如第 8 章从 Modern C++ 换成了「类与多态」，续读到第 20 页毫无意义
      if (slideCount != null && p.total != null && p.total !== slideCount) return 0;
      return p.slide;
    },
